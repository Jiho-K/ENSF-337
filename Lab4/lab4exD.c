 ...
 */
 ...
 //please note I have changed the name of function from my_strncmp to my_strcmp!!!
...
int my_strlen(const char *s)
{
	int i = 0;
	while(*(s+i) != '\0' && *(s+i) != '\n'){
		//this makes sure that when string ends or hits new sentence, loop ends.
		i++;
	}		
	return i;
}
...
void my_strncat(char *dest, const char *source, int n)
{
	...
	*(dest+y+n) = '\0';
	//do NOT remove this line. This ensures that there is no garbage values following the array of char that is desired.
}
...
int my_strcmp(const char* str1, const char* str2)
{
	...
	return (xnum - ynum);
	//note that this can very simply be changed to return 0 or 1 or -1 by using an if statement and setting the conditions for when s1 is less than, greater than, or equal to s2.
}
